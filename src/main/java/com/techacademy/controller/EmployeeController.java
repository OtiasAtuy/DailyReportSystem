...
public class EmployeeController {
 ...
 // 従業員更新処理
    ...
    public String update(@PathVariable("code") String code, @Validated Employee employee, BindingResult res, Model model) {
        Employee serviceEmployee = employeeService.findByCode(code);
        // パスワード空白チェック
        // パスワードが空白だった場合、既存のパスワードを設定
        if ("".equals(employee.getPassword())) {
            employee.setPassword(serviceEmployee.getPassword());
        } else {
            // パスワードが入力されている場合はチェックを実行
            ErrorKinds result = employeeService.employeePasswordCheck(employee);

            if (result == ErrorKinds.HALFSIZE_ERROR) {
                res.rejectValue("password", "error.password", "パスワードは半角英数字のみで入力してください");
            }
            if (result == ErrorKinds.RANGECHECK_ERROR) {
                res.rejectValue("password", "error.password", "8文字以上16文字以下で入力してください");
            }
        }
