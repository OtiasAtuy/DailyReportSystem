...
public class EmployeeService {
 ...
 // 従業員更新
    ...
    public ErrorKinds update(Employee employee) {
        //　.orElseThrow()を付けることでfindById()の戻り値であるOptional<Employee>をEmployeeに変換して型を揃える
        Employee updateEmployee = employeeRepository.findById(employee.getCode()).orElseThrow(() -> new RuntimeException("従業員のデータが見つかりません"));

        // 更新前のEmployeeをベースに上書きしていき、保存する
        updateEmployee.setName(employee.getName());
        updateEmployee.setPassword(employee.getPassword());
        updateEmployee.setRole(employee.getRole());

        LocalDateTime now = LocalDateTime.now();
        updateEmployee.setCreatedAt(updateEmployee.getCreatedAt());
    ...
    public ErrorKinds delete(String code, UserDetail userDetail) {
        ...
        // 日報が従業員テーブルを参照しているため、最後に削除フラグの処理をする
        LocalDateTime now = LocalDateTime.now();
        employee.setUpdatedAt(now);
